...
{
	printf("%s : %d %x %d %d\n",__FUNCTION__,__LINE__,(tlsfptr_t)mem,ALIGN_SIZE,(tlsfptr_t)mem % ALIGN_SIZE);
...
void *juicevm_tlsf_p = 0;
void *juicevm_tlsfmem_mem_p = 0;
static unsigned char juicevm_mem_pool_buf[1 * 1024 * 1024];
void juicevm_mem_init(void){
	printf("%s : %d\n",__FUNCTION__,__LINE__);
	
	printf("%s : %d sizeof(juicevm_mem_pool_buf) %d\n",__FUNCTION__,__LINE__,sizeof(juicevm_mem_pool_buf));
	printf("%s : %d juicevm_mem_pool_buf : %p\n",__FUNCTION__,__LINE__,juicevm_mem_pool_buf);
    juicevm_tlsf_p = tlsf_create_with_pool(juicevm_mem_pool_buf, sizeof(juicevm_mem_pool_buf));
	printf("%s : %d %x\n",__FUNCTION__,__LINE__,juicevm_tlsf_p);
    // if(tlsf_add_pool(juicevm_tlsf_p, juicevm_mem_pool_buf, sizeof(juicevm_mem_pool_buf)) == 0){
    //     return;
    // }
}

void* juicevm_malloc(size_t bytes)
{
    if(!bytes){
        return 0;
    }
    ...
    m = tlsf_malloc(juicevm_tlsf_p, bytes);
	// printf("%s : %d\n",__FUNCTION__,__LINE__);
...
void* juicevm_realloc(void* ptr, size_t size)
{
    return tlsf_realloc(juicevm_tlsf_p, ptr, size);

}

void* juicevm_calloc(size_t nelem, size_t elem_size)
{
    void * m;
	if((m = tlsf_malloc(juicevm_tlsf_p,nelem * elem_size)))
        memset(m, 0, nelem * elem_size);
	return m;
}

void juicevm_free(void* p){
    tlsf_free(juicevm_tlsf_p, p);
}

void juicevm_mem_deinit(void){
    tlsf_destroy(juicevm_tlsfmem_mem_p);
}